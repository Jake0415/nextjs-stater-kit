...
**A. 내부 DB 모드** (기존):
- `front-end/prisma/` 존재 → Prisma 사용
- `front-end/lib/db/schema.ts` 존재 → Drizzle 사용
- `@supabase/supabase-js` 설치 → Supabase 사용

**B. 외부 API 모드** (FastAPI 등):
- `front-end/lib/api/client.ts` 존재 → 외부 API 모드
- `docs/API-ENDPOINTS.md` 존재 → 엔드포인트 명세 참조

감지 실패 시 → 사용자에게 선택 요청

기존 API 라우트(`front-end/app/api/`) 및 Server Actions(`front-end/lib/actions/`) 확인 후 재사용 가능한 코드 파악
...
**API Routes** (`front-end/app/api/` 하위):
- RESTful 패턴 준수
- 입력 검증: Zod 스키마 적용
- 에러 처리: NextResponse로 적절한 상태 코드 반환

**Server Actions** (`front-end/lib/actions/` 하위):
- `"use server"` 지시어 포함
- 입력 검증: Zod 스키마 적용
