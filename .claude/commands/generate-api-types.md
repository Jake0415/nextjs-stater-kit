...
- PRD의 "데이터 모델" 섹션 추출
- 기존 도메인 타입 확인: `front-end/lib/types/index.ts`
...
- `front-end/lib/types/index.ts` — 기존 도메인 타입
- `front-end/lib/api/types.ts` — 기존 API DTO 타입 (있는 경우)
- `front-end/lib/validations/` — 기존 Zod 스키마 (있는 경우)
- 중복 방지: 기존 타입과 겹치는 부분 식별

### 3단계: API DTO 타입 생성

**`front-end/lib/api/types.ts`** 에 다음을 생성:
...
**생성 규칙**:
- 도메인 모델(`front-end/lib/types/`)과 동일한 구조면 재사용 (re-export)
- API에서만 사용되는 타입(DTO)은 `front-end/lib/api/types.ts`에 정의
- 요청 DTO는 `Partial<>`, `Pick<>`, `Omit<>` 활용으로 간결하게

### 4단계: Zod 검증 스키마 생성

**`front-end/lib/validations/`** 하위에 도메인별 파일 생성:

```
front-end/lib/validations/
...
**`front-end/lib/api/endpoints.ts`** 에 API 엔드포인트 상수를 생성:
...
- `front-end/lib/types/index.ts`와의 중복 여부 확인
